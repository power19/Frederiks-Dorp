...
async function main() {
  ...
  await prisma.changelogEntry.upsert({
    where: { id: "seed-log-starlink" },
    update: {},
    create: {
      id: "seed-log-starlink",
      deviceId: starlink.id,
      action: "CREATED",
      summary: `Device "Starlink-Main" created`,
      author: admin.email,
    },
  });

  await prisma.changelogEntry.upsert({
    where: { id: "seed-log-router" },
    update: {},
    create: {
      id: "seed-log-router",
      deviceId: router.id,
      action: "CREATED",
      summary: `Device "Core-Router-01" created`,
      author: admin.email,
    },
  });
